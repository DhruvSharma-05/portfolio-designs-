...
const deliveries = (content) => (content.deliveries || []).filter((d) => d.code && d.folderId);
...
export async function resolveDelivery(drive, code) {
  ...
  const found = deliveries(content).find((d) => d.code.toLowerCase() === clean);
  ...
  return { ok: true, delivery: found, folderId };
}
